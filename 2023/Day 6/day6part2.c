...
int main(void)
{
    ...
    uint64_t race_time = combine_digits(time_component, num_comp);
    uint64_t winning_dist = combine_digits(winning_dist_component, num_comp);

    // d = t_held(t - t_held)
    uint64_t win_cnt = 0;
    for (uint64_t t_held = 0; t_held <= race_time; ++t_held)
    {
        uint64_t dist_traveled = t_held * (race_time - t_held);
        if (dist_traveled > winning_dist)
        {
            ++win_cnt;
        }
    }
